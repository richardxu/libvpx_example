...
static RK_S32 vpu_encode_demo(VpuApiDemoCmdContext_t *cmd)
{
    ...
    if (cmd->have_output) {
        VPU_DEMO_LOG("vpu api demo output file: %s\n",
                     cmd->output_file);
        pOutFile = fopen((const char*)cmd->output_file, "wb");
        if (pOutFile == NULL) {
    ...
    */
    ...
    if (pOutFile && (ctx->extradata_size > 0) && (ctx->videoCoding == OMX_RK_VIDEO_CodingAVC)) {
...
static RK_S32 (*rkvpu_open_ctx)(VpuCodecContext_t **ctx);
static RK_S32 (*rkvpu_close_ctx)(VpuCodecContext_t **ctx);
static void *rkapi_hdl = NULL;
...
static RK_S32 vpu_decode_demo(VpuApiDemoCmdContext_t *cmd)
{
    ...
    FILE* pOutFile = NULL;
    VpuCodecContext_t *ctx = NULL;
    RK_S32 fileSize = 0, pkt_size = 0, ret = 0;
    ...
    RK_U8* pExtra = NULL;
    RK_U32 extraSize = 0;
    RK_U32 check_new_fw = !access("/dev/rkvdec", F_OK);

    if (check_new_fw) {
        rkapi_hdl = dlopen("/system/lib/libvpu.so", RTLD_LAZY);
        if (rkapi_hdl == NULL) {
            ALOGE("new framework dlopne vpu fail decode no process");
            return 0;
        }
        rkvpu_open_ctx = (RK_S32 (*)(VpuCodecContext_t **ctx))dlsym(rkapi_hdl, "vpu_open_context");
        rkvpu_close_ctx = (RK_S32 (*)(VpuCodecContext_t **ctx))dlsym(rkapi_hdl, "vpu_close_context");
    }
    ...
    if (check_new_fw) {
        ctx = (VpuCodecContext_t*)malloc(sizeof(VpuCodecContext_t));
        memset(ctx, 0, sizeof(VpuCodecContext_t));
        ctx->videoCoding = cmd->coding;
        ctx->width = cmd->width;
        ctx->height = cmd->height;
        ret = rkvpu_open_ctx(&ctx);
    } else {
        ret = vpu_open_context(&ctx);
    }
    if (ret || (ctx == NULL)) {
